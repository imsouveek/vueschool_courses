...
const validateFile = (file: File): boolean => {
    ...
    if (file.size > props.maxSizeMB * 1024 * 1024) {
        setError(`File ${file.name} is too large. Maximum size is ${props.maxSizeMB}MB`)
        return false
    }
...
const uploadFile = async (fileStatus: FileStatus) => {
    ...
    try {
        ...
        emit(
            'upload-error',
            error instanceof Error ? error : new Error(fileStatus.error || 'Upload failed')
        )
    }
}
</script>
...
<template>
    <div class="w-full">
        ...
        <!-- File List -->
        <FileList :files="selectedFiles" :preview="preview" :inline="inline" @remove="removeFile" />
    </div>
</template>
